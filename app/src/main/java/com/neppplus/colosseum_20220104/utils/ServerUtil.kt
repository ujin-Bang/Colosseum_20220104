...
//            1.어디로 가야 하는지? url + 어떤 파라미터 데이터? 같이 작성

//            url을 만드는 과정이 복잡함. => 한간계씩 쌓아나가는 식으로 URL 작성

            val urlBuilder = "${HOST_URL}/user_check".toHttpUrlOrNull()!!.newBuilder() // 서버 주소 /기능주소까지만작성
...
//        연습 - 내 정보 가져오기 (토큰 첨부) -GET
        fun getRequestMyInfo( context: Context, handler: JsonResponseHandler? ) {
            ...
            val urlBuilder = "${HOST_URL}/user_info".toHttpUrlOrNull()!!.newBuilder() // 서버 주소 /기능주소까지만작성
//            urlBuilder.addEncodedQueryParameter("type",type)
//            urlBuilder.addEncodedQueryParameter("value", value)
...
//      메인화면의 데이터 가져오기(토론 주제 목록 가져오기) - GET
        fun getRequestMainInfo( context: Context, handler: JsonResponseHandler? ) {
            ...
            val urlBuilder = "${HOST_URL}/v2/main_info".toHttpUrlOrNull()!!.newBuilder() // 서버 주소 /기능주소까지만작성
//            urlBuilder.addEncodedQueryParameter("type",type)
//            urlBuilder.addEncodedQueryParameter("value", value)
...
//      토론 상세 조회하기 - GET
        fun getRequestTopicDetail( context: Context, topicId: Int, orderType: String,  handler: JsonResponseHandler? ) {
            ...
            val urlBuilder = "${HOST_URL}/topic".toHttpUrlOrNull()!!.newBuilder() // 서버 주소 /기능주소까지만작성

//          주소 양식 : Path - /topic/3 => /3 PathSegment라고 부름.
//          주소 양식 : Query - /topic?name=조경진  QueryParametr라고 부름
...
//        댓글 상세 조회(+대댓글 목록)
    ...
    val urlBuilder = "${HOST_URL}/topic_reply".toHttpUrlOrNull()!!.newBuilder() // 서버 주소 /기능주소까지만작성
